...
class DAGGDetect(Detect):
    ...
    def __init__(
        ...
        """Initialize the detection head plus a P3 auxiliary heatmap predictor."""
        if not ch:
        ...
        super().__init__(nc=nc, reg_max=reg_max, end2end=bool(end2end), ch=ch)
    ...
    def forward(self, x):
        """Attach Gaussian logits to training predictions and keep inference identical to Detect."""
        dagg_logits = self.dagg_head(x[0]) if self.training else None
        preds = super().forward(x)
        if self.training:
            preds["dagg_logits"] = dagg_logits
        return preds
