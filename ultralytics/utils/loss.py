...
class BboxLoss(nn.Module):
    ...
    def __init__(self, reg_max: int = 16, iou_loss: str = "ciou", inner_iou_ratio: float = 0.7):
...
class DAGGDetectionLoss(v8DetectionLoss):
    ...
    def __init__(self, model):
        ...
        self.loss_names = (*self.loss_names, "dagg_loss")
    ...
    def __call__(self, preds, batch):
        """Calculate detection and DAGG losses using the current dictionary-based prediction API."""
        parsed = self.parse_output(preds)
        dagg_logits = parsed.get("dagg_logits")
        ...
        return total_loss, {**loss_items, "dagg_loss": dagg_loss.detach()}
